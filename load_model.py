...
def load_model(model_name, number_of_class, pre_train_flag=False, freeze_feature_arg=False, device_l=torch.device('cpu')):
    device = device_l
    new_model = None
    if model_name == 'vgg16' or model_name == 'vgg13' or model_name == 'vgg11':
        if model_name == 'vgg11':
            new_model = torchvision.models.vgg11(pretrained=pre_train_flag)
        if model_name == 'vgg13':
            new_model = torchvision.models.vgg13(pretrained=pre_train_flag)
        if model_name == 'vgg16':
            new_model = torchvision.models.vgg16(pretrained=pre_train_flag)
            print('VGG16 Loaded')
    ...
    if model_name == 'vgg16bn' or model_name == 'vgg13bn' or model_name == 'vgg11bn':
        if model_name == 'vgg11bn':
            new_model = torchvision.models.vgg11_bn(pretrained=pre_train_flag)
        if model_name == 'vgg13bn':
            new_model = torchvision.models.vgg13_bn(pretrained=pre_train_flag)
        if model_name == 'vgg16bn':
            new_model = torchvision.models.vgg16_bn(pretrained=pre_train_flag)
        if freeze_feature_arg:
            for param in new_model.parameters():
                param.requires_grad = False
        # Need to change the below code if we choose different model
...
def freeze_feature_by_conv_idx(model, conv_idx):
    for l, param in zip(model.features, model.parameters()):
        if str(model.features[l]).find('Conv') != -1:
            param.requires_grad =False


# In[9]: unfreeze all the weights
def unfreeze_feature(model, conv_idx):
    for l, param in zip(model.features, model.parameters()):
        if str(model.features[l]).find('Conv') != -1:
            param.requires_grad =True
            
            
def unfreeze(model, convolution_layer_index):
    length = len(model.features)
    print('printing from unfreeze')
    for l, param in zip(range(length), model.parameters()):
        if l not in convolution_layer_index:
            param.requires_grad = True
